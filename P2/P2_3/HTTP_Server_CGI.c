...
uint32_t netCGI_Script (const char *env, char *buf, uint32_t buflen, uint32_t *pcgi) {
  ...
  switch (env[0]) {
    ...
    case 'g':
      ...
      break;
		case 'h':
			// RTC Data para la nueva página web
			switch (env[2]) {
				case '1': // Hora
					RTC_CalendarShow(aShowTime, aShowDate); // Actualiza los buffers
					len = (uint32_t)sprintf (buf, &env[4], (char *)aShowTime);
					break;
				case '2': // Fecha
					len = (uint32_t)sprintf (buf, &env[4], (char *)aShowDate);
					break;
			}
			break;
    case 'x':
